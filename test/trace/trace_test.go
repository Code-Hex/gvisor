...
func checkSyscallRead(msg test.Message) error {
	...
	if p.HasOffset {
		// Workload always uses 20 for read offsets.
		if want := uint64(20); want != p.Offset {
			return fmt.Errorf("invalid offset, want: %d, got: %d", want, p.Offset)
		}
	} else if p.Offset != 0 {
...
func checkSyscallWrite(msg test.Message) error {
	...
	if p.HasOffset {
		// Workload always uses 10 for write offsets.
		if want := uint64(10); want != p.Offset {
			return fmt.Errorf("invalid offset, want: %d, got: %d", want, p.Offset)
		}
	} else if p.Offset != 0 {
		return fmt.Errorf("offset should be 0: %+v", p)
