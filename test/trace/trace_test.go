...
func checkSyscallRead(msg test.Message) error {
	...
	if p.HasOffset {
		...
		if lower, upper := uint64(20), uint64(120); p.Offset < lower && p.Offset > upper {
...
func checkSyscallWrite(msg test.Message) error {
	...
	if p.HasOffset {
		// Workload always uses 10 for read offsets (account for partial reads).
		if lower, upper := uint64(10), uint64(110); p.Offset < lower && p.Offset > upper {
			return fmt.Errorf("invalid offset, want: [%d, %d], got: %d", lower, upper, p.Offset)
		}
	} else if p.Offset != 0 {
		return fmt.Errorf("offset should be 0: %+v", &p)
